import requests
import os
import json
import argparse
from datetime import datetime, timezone
from dotenv import load_dotenv
from defi_positions import get_defi_positions
from alerts import check_alerts

load_dotenv()

REGISTRY = {
    "ETH":     {"risk_level": 10},
    "WETH":    {"risk_level": 10},
    "WBTC":    {"risk_level": 20},
    "USDC":    {"risk_level": 15},
    "USDT":    {"risk_level": 20},
    "DAI":     {"risk_level": 15},
    "SUSD":    {"risk_level": 25},
    "AUSD":    {"risk_level": 25},
    "MKUSD":   {"risk_level": 20},
    "CRV":     {"risk_level": 45},
    "3CRV":    {"risk_level": 35},
    "ARB":     {"risk_level": 40},
    "OP":      {"risk_level": 40},
    "MATIC":   {"risk_level": 40},
    "SYRUP":   {"risk_level": 50},
    "PERQ":    {"risk_level": 55},
    "SHFL":    {"risk_level": 65},
    "DJIA":    {"risk_level": 60},
    "GBR":     {"risk_level": 70},
    "TREE":    {"risk_level": 70},
    "PEANUT":  {"risk_level": 80},
    "GME":     {"risk_level": 85},
    "DOGENES": {"risk_level": 85},
    "REMILIA": {"risk_level": 80},
    "TISM":    {"risk_level": 80},
    "SHINSHU": {"risk_level": 80},
    "DEFAULT": {"risk_level": 75},
}

CHAIN_RISK = {
    "ethereum": 10,
    "arbitrum": 25,
    "optimism": 25,
    "polygon":  35,
    "bsc":      45,
}

def parse_args():
    parser = argparse.ArgumentParser(description="DeFi Risk Engine")
    parser.add_argument("--wallet",  type=str, action="append", default=None, help="Wallet address (repeatable)")
    parser.add_argument("--wallets", type=str, default=None,                  help="Path to .txt file with wallet addresses")
    parser.add_argument("--output",  type=str, default="risk_report.json",    help="Output JSON file")
    parser.add_argument("--silent",  action="store_true",                     help="Suppress console output")
    return parser.parse_args()

def load_last_report(wallet: str) -> dict | None:
    if not os.path.exists("reports"):
        return None
    slug = wallet[:8]
    files = sorted([f for f in os.listdir("reports") if slug in f])
    if len(files) < 2:
        return None
    with open(f"reports/{files[-2]}") as f:
        return json.load(f)

def scan_wallet(wallet: str, headers: dict, headers_sim: dict, silent: bool = False) -> dict:
    def log(*a, **kw):
        if not silent:
            print(*a, **kw)

    last_report = load_last_report(wallet)

    response = requests.get(
        f"https://api.sim.dune.com/v1/evm/balances/{wallet}?chain_ids=1,42161,10,137",
        headers=headers
    ).json()

    balances = response["balances"]
    balances = [t for t in balances if t.get("value_usd") is not None]
    total = sum(t["value_usd"] for t in balances)

    CR = PR = CoR = ChR = 0.0

    for token in balances:
        sym   = (token.get("symbol") or token.get("token_symbol") or "DEFAULT").upper()
        chain = (token.get("chain") or "unknown").lower()
        info  = REGISTRY.get(sym, REGISTRY["DEFAULT"])
        pct   = token["value_usd"] / total

        CR  += info["risk_level"] * pct
        PR  += info["risk_level"] * pct
        CoR += 100 * pct if pct > 0.5 else 0
        ChR += CHAIN_RISK.get(chain, 50) * pct

        log(f"[{sym}] chain={chain} | {round(pct*100,2)}% | risk={info['risk_level']}")

    portfolio_risk = 0.35*CR + 0.25*PR + 0.25*CoR + 0.15*ChR

    log(f"\nCR:  {round(CR,2)}")
    log(f"PR:  {round(PR,2)}")
    log(f"CoR: {round(CoR,2)}")
    log(f"ChR: {round(ChR,2)}")
    log(f"==> Portfolio Risk Score: {round(portfolio_risk,2)}")

    log("\n--- DeFi Positions ---")
    defi = get_defi_positions(wallet, headers_sim)
    DR   = defi["DR_score"]

    if defi["total_usd"] > 0:
        token_weight = total / (total + defi["total_usd"])
        defi_weight  = defi["total_usd"] / (total + defi["total_usd"])
        unified_risk = round(portfolio_risk * token_weight + DR * defi_weight, 2)
    else:
        unified_risk = round(portfolio_risk, 2)

    log(f"\n{'='*40}")
    log(f"Token Portfolio Risk:  {round(portfolio_risk,2)}")
    log(f"DeFi Positions Risk:   {DR}")
    log(f"==> UNIFIED RISK v0.2: {unified_risk}")
    log(f"{'='*40}")

    # === TREND ===
    if last_report:
        prev = last_report["scores"]
        def delta(key, curr):
            d = round(curr - prev.get(key, curr), 2)
            arrow = "▲" if d > 0 else "▼" if d < 0 else "="
            return f"{arrow} {abs(d)}"

        log(f"\n--- Trend vs last scan ({last_report['timestamp'][:16]}) ---")
        log(f"Unified Risk: {unified_risk}  ({delta('unified_risk', unified_risk)})")
        log(f"CR:           {round(CR,2)}  ({delta('CR', round(CR,2))})")
        log(f"CoR:          {round(CoR,2)}  ({delta('CoR', round(CoR,2))})")
        log(f"ChR:          {round(ChR,2)}  ({delta('ChR', round(ChR,2))})")
    else:
        log("\n--- Trend: brak poprzedniego raportu ---")

    # === ALERTS ===
    alert_tokens = [
        {
            "symbol":  (t.get("symbol") or "DEFAULT").upper(),
            "percent": round((t["value_usd"] / total) * 100, 2),
        }
        for t in balances
    ]
    alerts = check_alerts(unified_risk, last_report, alert_tokens)

    log("\n--- Alerts ---")
    if alerts:
        for a in alerts:
            log(a)
    else:
        log("✅ No critical alerts detected.")

    # === SAVE ===
    report = {
        "wallet": wallet,
        "timestamp": datetime.now(timezone.utc).isoformat(),
        "scores": {
            "CR": round(CR, 2),
            "PR": round(PR, 2),
            "CoR": round(CoR, 2),
            "ChR": round(ChR, 2),
            "portfolio_risk": round(portfolio_risk, 2),
            "defi_risk": DR,
            "unified_risk": unified_risk,
        },
        "risk_label": "🟢 LOW" if portfolio_risk < 30 else "🟡 MEDIUM" if portfolio_risk < 60 else "🔴 HIGH",
        "alerts": alerts,
        "tokens": [
            {
                "symbol": (t.get("symbol") or "DEFAULT").upper(),
                "chain":  t.get("chain", "unknown"),
                "value_usd": round(t["value_usd"], 4),
                "percent":   round((t["value_usd"] / total) * 100, 2),
            }
            for t in balances
        ],
    }

    os.makedirs("reports", exist_ok=True)
    slug = datetime.now(timezone.utc).strftime("%Y-%m-%dT%H-%M-%S")
    history_path = f"reports/{slug}_{wallet[:8]}.json"
    with open(history_path, "w") as f:
        json.dump(report, f, indent=2)

    log(f"\n📁 History saved → {history_path}")

    return report


def main():
    args = parse_args()

    headers     = {"X-SIM-API-Key": os.getenv("SIM_API_KEY")}
    headers_sim = {"X-Sim-Api-Key": os.getenv("SIM_API_KEY")}

    # Collect wallets
    wallets = []
    if args.wallets:
        with open(args.wallets) as f:
            wallets = [line.strip() for line in f if line.strip()]
    if args.wallet:
        wallets += args.wallet
    if not wallets:
        wallets = [os.getenv("WALLET", "0x6cd68e8f04490cd1a5a21cc97cc8bc15b47dc9eb")]

    reports = []
    for wallet in wallets:
        if not args.silent:
            print(f"\n{'#'*40}")
            print(f"# Wallet: {wallet}")
            print(f"{'#'*40}")
        report = scan_wallet(wallet, headers, headers_sim, silent=args.silent)
        reports.append(report)

    # Save latest (single or last wallet)
    with open(args.output, "w") as f:
        json.dump(reports if len(reports) > 1 else reports[0], f, indent=2)
    print(f"\n✅ Report saved → {args.output}")

    # Multi-wallet summary
    if len(reports) > 1:
        print(f"\n{'='*40}")
        print("MULTI-WALLET SUMMARY")
        print(f"{'='*40}")
        for r in reports:
            score = r["scores"]["unified_risk"]
            label = r["risk_label"]
            alert_count = len(r["alerts"])
            flag = f" ⚠️  {alert_count} alert(s)" if alert_count else ""
            print(f"{r['wallet'][:10]}...  Risk: {score}  {label}{flag}")


if __name__ == "__main__":
    main()
